...
1. **Record** — two threads capture in parallel: the mic (`sounddevice`, 16 kHz
   ...
   device's *native* format because loopback does no resample/downmix on its
   own).
...
3. **Transcribe** — WhisperX `medium` model, language `pt`, then timestamp
   alignment.
4. **Diarize** — `pyannote` pipeline (needs a Hugging Face token) assigns a
...
Python · Tkinter · pyaudiowpatch (WASAPI loopback) · sounddevice · scipy ·
WhisperX · pyannote
